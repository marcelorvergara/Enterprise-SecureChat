...
| **M16** | ✅ Complete | AI-Powered Suggested Follow-ups: Claude instructed via `JSON_FORMAT_INSTRUCTION` appended to both `buildSystemPrompt()` variants to return a structured `{"answer":"...","suggestions":[...]}` object. `parseClaudeResponse()` strips the JSON object bounds (`indexOf('{')` / `lastIndexOf('}')`) before parsing to handle accidental markdown fences; falls back to raw text + empty suggestions on any parse failure. Each suggestion string passes through `dlpClient.analyze()` independently before reaching the client — suggestions never bypass DLP. `STRUCTURED_CHAT_MAX_TOKENS = 1536` calls the existing 3-arg `complete()` overload without raising the default 1024 (Constraint #10). `ChatResponse` record gains a `List<String> suggestions` field. Angular `suggestions: string[]` state array rendered as a `<mat-chip-set>` row globally between the message thread and the composer (`@if (suggestions.length > 0)` — not inside the `*ngFor` loop); clicking a chip calls `submitSuggestion()` which clears chips, populates the input, and submits. `ng2-charts@6` + `chart.js@4.4.0` installed with `--legacy-peer-deps`; `"skipLibCheck": true` added to `tsconfig.json` to resolve ng2-charts v6 type declaration import issue. Three new `RagServiceTest` cases (valid JSON suggestions, malformed JSON fallback, DLP called ≥3× per request). |
| **M17** | ✅ Complete | Admin Security Heatmap: `RestrictionAuditLogRepository.findTopRestrictedPaths()` — native PostgreSQL `unnest(restricted_paths) + COUNT(*)` query returning top 20 blocked paths. `MessageRepository.findDlpDensityByDay()` — native `DATE_TRUNC('day') + SUM(dlp_redacted)` query returning last 30 days of DLP trigger density. New `SecurityHeatmapResponse` record DTO with nested `PathHitCount` + `DlpDensityPoint` records. `GET /api/admin/metrics/security-heatmap` added to `AdminController` (covered by existing class-level `@PreAuthorize("hasRole('admin')")`); `MessageRepository` injected via constructor. Angular admin panel adds two `<canvas baseChart>` charts in a CSS grid — horizontal bar (FGA blocks, red) and vertical bar (DLP density, orange). `Chart.register(...registerables)` called globally in `app.config.ts` before `appConfig`. `loadHeatmap()` called from `ngOnInit()`. New `AdminControllerTest` (`@WebMvcTest` + `@Import(SecurityConfig.class)`) with 3 security tests (401 unauthenticated, 403 employee, 200 admin). No raw prompt data is read or exposed — Constraint #4 compliant. |
| **M18** | ✅ Complete | FGA Classification Security Audit (CI/CD): M15 pentest script promoted to a first-class CI gate. `ingestion/tests/test_classification_fga_integration.py` — full pytest refactor with `pytest.mark.integration` marker, session-scoped `ingested_points` fixture (ingest 3 XLSX fixtures once, delete from Qdrant in teardown via `POST /points/delete` with `doc_id` filter), and 4 test classes mirroring the original 4 validation steps (`TestPayloadAudit`, `TestLowClearanceIsolation`, `TestHighClearanceElevation`, `TestIdEnumerationAttack`). Module-level `pytestmark` conditional skip when `QDRANT_URL`/`QDRANT_API_KEY` are absent — bare `pytest` prints a skip notice instead of failing. `ingestion/pytest.ini` registers the `integration` marker with `--strict-markers`. `.github/workflows/security-audit.yml` — triggers on push/PR to `main` for `ingestion/src/**` or `backend/**/fga/**` changes; caches `~/.cache/huggingface` keyed on `requirements.txt` hash (~90 MB `all-MiniLM-L6-v2` model); starts uvicorn in background, polls `/health` up to 2 min, runs pytest, kills process with `if: always()`. `Makefile` at project root — `make security-audit` (starts/stops ingestion service, guards on missing env vars) and `make security-audit-fixtures` for local developer use. Root-level `pentest_classification_sync.py` deleted (had hardcoded credentials, no cleanup). |
